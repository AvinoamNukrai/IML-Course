...
class GradientDescent:
    ...
    def fit(self, f: BaseModule, X: np.ndarray, y: np.ndarray):
        ...
        curr_w,delta_, iteration, sum_w, min_w, \
        min_obj = f.weights, self.tol_+1, 0, f.weights,\
                  f.weights, f.compute_output(X=X,y=y)

        while iteration < self.max_iter_ and delta_ > self.tol_:
            eta_ = self.learning_rate_.lr_step(t=iteration)
            val_ = f.compute_output(X=X, y=y)
            grad_ = f.compute_jacobian(X=X, y=y)
            last_w = f.weights
            f.weights = last_w - eta_ * grad_
            delta_ = np.linalg.norm(f.weights - last_w)
            self.callback_(solver=self, weights=curr_w, val=val_, grad=grad_, t=iteration, eta=eta_, delta_=delta_)
            sum_w += f.weights
            # if self.out_type_=="best" and val_<min_obj :
            #     min_w = f.weights
            #     min_obj = val_
            iteration += 1
            curr_w = f.weights
